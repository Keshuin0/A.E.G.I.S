...
use aegis_runtime::{Block, EXISTENTIAL_DEPOSIT};
use sp_keyring::Sr25519Keyring;

impl SubstrateCli for Cli {
    fn impl_name() -> String { "AEGIS Sovereign Node".into() }
    ...
    match &cli.subcommand {
                ...
                match cmd {
                    BenchmarkCmd::Pallet(cmd) => {
                        if !cfg!(feature = "runtime-benchmarks") {
                            return Err("Runtime benchmarking wasn't enabled.".into());
                        }
                    ...
                    BenchmarkCmd::Extrinsic(cmd) => {
                        ...
                        let ext_factory = ExtrinsicFactory(vec![
                            ...
                            Box::new(TransferKeepAliveBuilder::new(
                                client.clone(), Sr25519Keyring::Alice.to_account_id(), EXISTENTIAL_DEPOSIT
                            )),
                        ]);
        ...
        None => {
            ...
            runner.run_node_until_exit(|config| async move {
                service::new_full::<sc_network::Litep2pNetworkBackend>(config).map_err(sc_cli::Error::Service)
            })
        },
    }
}